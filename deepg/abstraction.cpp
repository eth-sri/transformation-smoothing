...
pair<double, Polyhedra> computePixelPolyhedra(
    ...
    const float radiusDecrease = 2;
    if (radiusDecrease > 0) {
      const float radius = min(img.nRows, img.nCols) - 1 - 2 * radiusDecrease;
      const float radiusSq = radius*radius;
      if (pixel.x*pixel.x + pixel.y*pixel.y > radiusSq) {
        return {0, {{0, 0}, vector<double>(degree, 0), 0, vector<double>(degree, 0), 0, degree}};
      }
    }
...
vector<Polyhedra> abstractWithPolyhedra(
    ...
    size_t k = 0;
    vector<vector<vector<Polyhedra>>> retT(img.nRows);
    vector<vector<vector<Polyhedra>>> retTT(img.nRows);
    for (int r = 0; r < img.nRows; ++r) {
      retT[r] = vector<vector<Polyhedra>>(img.nCols);
      retTT[r] = vector<vector<Polyhedra>>(img.nCols);
      for (int c = 0; c < img.nCols; ++c) {
        retT[r][c] = vector<Polyhedra>(img.nChannels);
        retTT[r][c] = vector<Polyhedra>(img.nChannels);
        for (int i = 0; i < img.nChannels; ++i) {
          retT[r][c][i] = ret[k++];
        }
      }
    ...
    const size_t filter_size = 5; //MNIST:9, cifar: 5
    const float filter_sigma = 0.1; //mnist 2.0, cifar 0.1

    const int fs = filter_size / 2;
    float *G = new float[(fs+1) * (fs+1)];
    getGaussianFilter(G, filter_size, filter_sigma);
    ...
    for (int r = 0; r < img.nRows; ++r) {
      for (int c = 0; c < img.nCols; ++c) {
        for (int cc = 0; cc < img.nChannels; ++cc) {
          ...
          for (int i = -fs; i <= signed(fs); ++i)
            for (int j = -fs; j <= signed(fs); ++j) {
              const int r_ = r + i;
              const int c_ = c + j;
              if (r_ > 0 && r_ < signed(img.nRows) && c_ > 0 && c_ < signed(img.nCols)) {
                tmp += G[abs(i) * (fs+1) +  abs(j)] * retT[r_][c_][cc].it;
                }
                
                }

          Polyhedra prev = retT[r][c][cc];
          retTT[r][c][cc] = {tmp, prev.wLower, prev.biasLower, prev.wUpper, prev.biasUpper, prev.degree};
        }        
      }
    }

    vector<Polyhedra> retTTT;
    for (int r = 0; r < img.nRows; ++r) {
      for (int c = 0; c < img.nCols; ++c) {        
        for (int cc = 0; cc < img.nChannels; ++cc) {
          retTTT.push_back(retTT[r][c][cc]);
        }
      }
    }
